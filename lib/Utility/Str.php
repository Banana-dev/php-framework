...
class Str
{
    ...
    /**
     * Returns a table classname
     *
     * @param string $table Table name
     * @param bool $namespace Flag to prefix the classname by App\Model\Table namespace
     *
     * @return string
     */
    public static function tableName(string $table, bool $namespace = false)
    {
        $out = self::camelize(self::pluralize($table) . '_collection', true);
        if ($namespace) {
            $out = 'App\\Model\\Table\\' . $out;
        }
        return $out;
    }
